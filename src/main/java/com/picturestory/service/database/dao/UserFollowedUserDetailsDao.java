 ...
 */
public class UserFollowedUserDetailsDao implements IUserFollowedUserDetailsDao<Integer,User>  {
    private IDataAccessAdapter mSolrAdapter;
    private ResponseData mResponseData;

    @Inject
    public UserFollowedUserDetailsDao(IDataAccessAdapter solrAdapter) {
        mSolrAdapter = solrAdapter;
        mResponseData = new ResponseData();
    }

    @Override
    public List<User> getAllFollowedUsers(Integer userId, int startIndex, int maxCount) {
        String query = String.format("q={!join from=%s to=%s}%s:%s&fq=%s:%s&%s=%s&%s=%s&%s", Constants.FOLLOWED_USER_ID,Constants.USER_ID,Constants.USER_ID,userId,Constants.USER_NAME,Constants.ALL,Constants.START,startIndex,Constants.ROWS,maxCount,Constants.WT_JSON);
    ...
    @Override
    public int fullCountAllFollowedUsers(Integer userId) {
        String query = String.format("q={!join from=%s to=%s}%s:%s&fq=%s:%s&%s", Constants.FOLLOWED_USER_ID,Constants.USER_ID,Constants.USER_ID,userId,Constants.USER_EMAIL,Constants.ALL,Constants.WT_JSON);
        ResponseData responseData = (ResponseData)mSolrAdapter.selectRequest(query);
        if (responseData.isSuccess()) {
