...
public class UserDetailsDao implements IUserDetailsDao<User>{
    ...
    @Override
    public boolean updateApNsIdOfUser(User user){
        JSONObject userJsonObject = new JSONObject();
        JSONObject setQuery = new JSONObject();
        String query = "";
        String recordId;
        ...
        try {
            setQuery = new JSONObject();
            setQuery.put(Constants.SET,user.getGcmId());
            userJsonObject.put(Constants.APNSID, setQuery);
            userJsonObject.put(Constants.ID, recordId);
            query = userJsonObject.toString();
            query = Constants.INSERT_START + query +Constants.INSERT_END;
            mResponseData = (ResponseData) mSolrAdapter.updateRequest(query);
    ...
    public boolean updateUserInterests(int userId,List<Integer> userInterests) {
        ...
        String query = "";
        String recordId;
        query = String.format("q=%s:%s AND %s:%s&%s", Constants.USER_ID,userId,Constants.USER_NAME,Constants.ALL, Constants.WT_JSON);
        ResponseData responseData = (ResponseData) mSolrAdapter.selectRequest(query);
        if (responseData.isSuccess()) {
            try {
                JSONObject userResponse = new JSONObject(responseData.getData());
        ...
        try {
            setQuery = new JSONObject();
            setQuery.put(Constants.SET,userInterests);
            userJsonObject.put(Constants.USER_INTERESTS, setQuery);
            userJsonObject.put(Constants.ID, recordId);
            query = userJsonObject.toString();
            query = Constants.INSERT_START + query +Constants.INSERT_END;
            mResponseData = (ResponseData) mSolrAdapter.updateRequest(query);
    ...
    @Override
    public boolean updateUserDescription(User user) {
        JSONObject userJsonObject = new JSONObject();
        JSONObject setQuery = new JSONObject();
        String query = "";
        String recordId;
        ...
        try {
            setQuery = new JSONObject();
            setQuery.put(Constants.SET,user.getUserDesc());
            userJsonObject.put(Constants.USER_DESCRIPTION, setQuery);
            userJsonObject.put(Constants.ID, recordId);
            query = userJsonObject.toString();
            query = Constants.INSERT_START + query +Constants.INSERT_END;
            mResponseData = (ResponseData) mSolrAdapter.updateRequest(query);
